"""Daemon service for HostInclusion nodes (FastAPI with automatic Zero-Dependency fallback)."""
...
# On Android Termux, search and pre-load any compiler-rt builtins to provide emulated TLS (__emutls_v) for Rust extensions
if "com.termux" in sys.executable or "TERMUX_VERSION" in os.environ or os.path.exists("/data/data/com.termux"):
    ...
    if os.path.isdir(lib_dir):
        for root, _, files in os.walk(lib_dir):
            for f in files:
                if f.startswith("libclang_rt.builtins") and f.endswith(".so"):
                    try:
...
def start_daemon(host: str = "0.0.0.0", port: int = 8765, log_level: str = "info") -> None:
    ...
    # Fallback to zero-dependency lightweight daemon
    from hostinclusion.lite_daemon import run_lite_daemon
    run_lite_daemon(host=host, port=port)


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="HostInclusion Daemon")
    parser.add_argument("--host", default="0.0.0.0", help="Bind host")
    parser.add_argument("--port", type=int, default=8765, help="Bind port")
    parser.add_argument("--log-level", default="info", help="Log level")
    args = parser.parse_args()
    start_daemon(host=args.host, port=args.port, log_level=args.log_level)
